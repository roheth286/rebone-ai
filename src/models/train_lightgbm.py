...
def train_lightgbm_model(train_path, model_output_path, random_state=42):
    # 1. Load data
    train_df = pd.read_csv(train_path)
    X_train = train_df.drop(columns=["Fracture"])
    y_train = train_df["Fracture"]
    
    print(f"Loaded training data for LightGBM: {X_train.shape}")
    ...
    print(f"LightGBM model saved successfully to: {model_output_path}")
