...
def evaluate_catboost_model(model_path, test_path, metrics_output_path, plots_dir):
    """
    Evaluates a trained CatBoost model on the test dataset.
    Finds the optimal decision threshold to maximize the F2-score,
    saves CatBoost specific plots, and exports JSON metrics alongside CatBoost hyperparameters.
    """
    ...
    # 5. Extract hyperparameters from CatBoost model
    model_params = model.get_params()
    selected_hyperparameters = {
        "iterations": int(model_params.get("iterations")) if model_params.get("iterations") is not None else None,
        "depth": int(model_params.get("depth")) if model_params.get("depth") is not None else None,
        "learning_rate": float(model_params.get("learning_rate")) if model_params.get("learning_rate") is not None else None,
        "random_seed": model_params.get("random_seed")
    }
    ...
    plt.title(f"CatBoost Confusion Matrix (Threshold={optimal_threshold:.2f})")
    plt.xlabel("Predicted Class")
    plt.ylabel("True Class")
    plt.savefig(os.path.join(plots_dir, "catboost_confusion_matrix.png"), dpi=200, bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(plots_dir, "catboost_roc_curve.png"), dpi=200, bbox_inches="tight")
    plt.close()
    
    # Plot C: Precision-Recall Curve
    plt.figure(figsize=(7, 6))
    plt.plot(recalls, precisions, color="firebrick", lw=2, label=f"PR Curve (AUC = {pr_auc:.2f})")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("CatBoost Precision-Recall Curve")
    plt.legend(loc="lower left")
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(plots_dir, "catboost_pr_curve.png"), dpi=200, bbox_inches="tight")
    plt.close()
    
    print(f"Saved CatBoost evaluation plots to: {plots_dir}")
