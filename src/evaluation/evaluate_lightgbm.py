...
def evaluate_lightgbm_model(model_path, test_path, metrics_output_path, plots_dir):
    """
    Evaluates a trained LightGBM model on the test dataset.
    Finds the optimal decision threshold to maximize the F2-score,
    saves LightGBM specific plots, and exports JSON metrics alongside LightGBM hyperparameters.
    """
    ...
    # 5. Extract hyperparameters from LightGBM model
    ...
    plt.title(f"LightGBM Confusion Matrix (Threshold={optimal_threshold:.2f})")
    plt.xlabel("Predicted Class")
    plt.ylabel("True Class")
    plt.savefig(os.path.join(plots_dir, "lightgbm_confusion_matrix.png"), dpi=200, bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(plots_dir, "lightgbm_roc_curve.png"), dpi=200, bbox_inches="tight")
    plt.close()
    
    # Plot C: Precision-Recall Curve
    plt.figure(figsize=(7, 6))
    plt.plot(recalls, precisions, color="darkgoldenrod", lw=2, label=f"PR Curve (AUC = {pr_auc:.2f})")
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("LightGBM Precision-Recall Curve")
    plt.legend(loc="lower left")
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(plots_dir, "lightgbm_pr_curve.png"), dpi=200, bbox_inches="tight")
    plt.close()
    
    print(f"Saved LightGBM evaluation plots to: {plots_dir}")
